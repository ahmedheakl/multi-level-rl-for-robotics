from highrl.graphs.model_training_plot_styles import train_stats_min_x
import pandas as pd
import os  

log_path = input("please Enter The logs path: ")
save_path = input("please enter the path to save figures: ")
x = int(input("please enter the number of points to get min of them: "))
...
for i,path in enumerate(models_log_files_pathes):
    ...
    name = files_names[i]+"_trail_{}_min_{}".format(i+1,x)
    name = os.path.join(save_graph_dir, name)
    train_stats_min_x (train_df,x,name)
