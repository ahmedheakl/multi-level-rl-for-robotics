...
import unittest
from highrl.utils.planner_checker import get_region_coordinates, check_valid_path_existance, check_if_point_inside_polygen, convex_hull_compute
...
class PlannerCheckerTest(unittest.TestCase):
    ...
    def test_valid_path_existance_true(self):
        ...
        value = check_valid_path_existance(obstacles, coords, 1280, 720, [2, 3], [4, 5], omit_first_four=False)
        expected = True
    ...
    def test_valid_path_existance_false(self):
        ...
        value = check_valid_path_existance(obstacles, coords, 6, 8, [2, 3], [4, 7], omit_first_four=False)
        expected = False
        self.assertEqual(expected, value, msg=f"\nExpected: {expected} || Found: {value}")
