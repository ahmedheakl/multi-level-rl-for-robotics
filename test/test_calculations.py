...
class TestCalculations(unittest.TestCase):
    ...
    def test_norm_floats(self):
        ...
        self.assertAlmostEqual(
            7.736924454, value, msg="Expected: {:.3f}, Found: {:.3f}".format(7.736924454, value))

    def test_difference_vectors(self):
        value = difference_vectors((3.6, 4.2), (2.5, 8.9))
        expected = (1.1, -4.7)
        self.assertEqual(
            expected, value, msg=f"Expected: {expected}, Found: {value}")

    def test_point_to_segment(self):
        tests = [[(1.0, 1.0), (1.0, 3.0), (2.0, 4.0), math.sqrt(2.0)],
                 [(1.0, 1.0), (1.0, 3.0), (2.0, 3.0), 1.0],
                 [(0.0, 0.0), (0.0, 3.0), (1.0, 1.0), 1.0]]
        for test in tests:
            value = point_to_segment_distance(test[0], test[1], test[2])
            expected = test[3]
            self.assertAlmostEqual(
                expected, value, msg="Expected: {:.3f}, Found: {:.3f}".format(expected, value))

    def test_point_to_point(self):
        tests = [[(1.0, 1.0), (2.0, 2.0), math.sqrt(2.0)],
