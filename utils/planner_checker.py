...
class PlannerChecker(object):
    ...
    def _construct_map(self, obstacles, new_obstacle=None):
        ...
        for current_obstacle in obstacles.obstacles_list:
            cx = current_obstacle.px
            cy = current_obstacle.py
            for x in range(cx, min(cx + current_obstacle.height, self.height)):
                for y in range(cy, min(cy + current_obstacle.width, self.width)):
                    self.map[x][y] = 'X'
